/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   enemy_update.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: apiscopo < apiscopo@student.42lausanne.    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/14 16:15:24 by apiscopo          #+#    #+#             */
/*   Updated: 2025/08/21 15:16:18 by apiscopo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../headers/cub3d.h"

static void	seek_velocity(t_enemy *e, double px, double py, double dt)
{
	double	dx;
	double	dy;
	double	len;
	double	acc;

	dx = px - e->x;
	dy = py - e->y;
	len = sqrt(dx * dx + dy * dy);
	if (len > 1e-6)
	{
		dx /= len;
		dy /= len;
	}
	acc = 8.0;
	e->vx += (dx * e->speed - e->vx) * acc * dt;
	e->vy += (dy * e->speed - e->vy) * acc * dt;
}

void	enemy_update(t_game *g, double dt)
{
	double	dx;
	double	dy;

	if (!g->enemy.alive)
		return ;
	if (dt <= 0.0)
		dt = 1.0 / 60.0;
	seek_velocity(&g->enemy, g->player.pos_x, g->player.pos_y, dt);
	g->enemy.x += g->enemy.vx * dt;
	g->enemy.y += g->enemy.vy * dt;
	enemy_resolve_collision(g);
	dx = g->player.pos_x - g->enemy.x;
	dy = g->player.pos_y - g->enemy.y;
	if (sqrt(dx * dx + dy * dy) <= g->enemy.radius)
		destroy_display(g, "You died\n", 0);
}
